...
| 服务 |  备注  |
| --- |  --- |
| 函数计算 FC |  word转pdf的函数需要部署到函数计算 |
| 对象存储 OSS |  原始word文件和转换后的pdf文件保存在对象存储OSS中 |
...
| 服务/业务 |  权限 |  备注  |
| --- |  --- |   --- |
| 函数计算 | AliyunFCFullAccess |  word转pdf的函数需要部署到函数计算 |
...
- 通过 [Serverless Devs Cli](https://www.serverless-devs.com/serverless-devs/install) 进行部署：
  ...
  - 初始化项目：`s init --project start-word2pdf-v3 -d start-word2pdf-v3`
...
项目部署完成，可以通过`invoke`命令或者在控制台使用事件进行触发/测试：
